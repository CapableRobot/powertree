...
- `examples/` — `sensor-board.kdl` (12V → 5V → LDO/buck, part library) and `ups.kdl` (adapter + battery, OR-ing, charger, harness, waiver)
- `tests/` — `pytest` (77 tests, including a parity check against the `ckdl` reference parser when it is installed)
...
| `loader.py` | `use` files, part merge, model build, reference resolution (layer 3) |
